...
import { Readable } from 'stream'; // Import Readable from 'stream' to convert web stream to Node.js stream
import { finished } from 'stream/promises'; // For a more modern way to handle stream completion
...
async function main() {
  ...
  const outDirSrc = path.resolve("static/ffmpeg", latestVersion);

  [outDirSrc].forEach((dir) => {
  ...
  await Promise.all([
    downloadFile(`${baseUrl}/ffmpeg-core.js`, path.join(outDirSrc, "ffmpeg-core.js")),
    downloadFile(`${baseUrl}/ffmpeg-core.wasm`, path.join(outDirSrc, "ffmpeg-core.wasm")),
  ]);
